...
class TestUser(unittest.TestCase):
    ...
    def test_get_users(self):
        ...
        user = self.tested.get_users()[0]

        request = responses.calls[0].request
        self.assertEqual(request.method, "GET")
        self.assertEqual(
            request.url,
            f"{self.tested.base_url}users?apiKey=key")
    ...
    def test_get_user(self):
        responses.add(
            responses.GET,
            f"{self.tested.base_url}users/1234567890",
            json={
                "id": 1234567890,
                "userId": "mike.green@test.jp",
                "name": "Mike Green",
        ...
        user = self.tested.get_user(1234567890)

        request = responses.calls[0].request
        self.assertEqual(request.method, "GET")
        self.assertEqual(
            request.url,
            f"{self.tested.base_url}users/1234567890?apiKey=key")
        self.assertEqual(user.id, 1234567890)
        self.assertEqual(user.user_id, "mike.green@test.jp")
        self.assertEqual(user.name, "Mike Green")
        self.assertEqual(user.role_type, 2)
    ...
    def test_get_own_user(self):
        responses.add(
            responses.GET,
            f"{self.tested.base_url}users/myself",
            json={
                "id": 1234567890,
                "userId": "mike.green@test.jp",
                "name": "Mike Green",
        ...
        user = self.tested.get_own_user()

        request = responses.calls[0].request
        self.assertEqual(request.method, "GET")
        self.assertEqual(
            request.url,
            f"{self.tested.base_url}users/myself?apiKey=key")
        self.assertEqual(user.id, 1234567890)
        self.assertEqual(user.user_id, "mike.green@test.jp")
        self.assertEqual(user.name, "Mike Green")
        self.assertEqual(user.role_type, 2)
